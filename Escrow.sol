...
/** @dev Example Escrow Wrote In Solidity For Better Context And Understanding */
...
contract Escrow {
    ...
    event Action(uint256 itemId, string actionType, Status status, address indexed executor);
    ...
    function createItem(string calldata purpose) external payable returns (bool) {
    ...
    function requestItem(uint256 itemId) external returns (bool) {
        ...
        emit Action(itemId, "REQUESTED", Status.OPEN, msg.sender);

        return true;
    }

    function approveRequest(uint256 itemId, address provider) external returns (bool) {
        ...
        emit Action(itemId, "APPROVED", Status.PENDING, msg.sender);
    ...
    function performDelievery(uint256 itemId) external returns (bool) {
        ...
        emit Action(itemId, "DELIVERY INTIATED", Status.DELIVERY, msg.sender);
    ...
    function confirmDelivery(uint256 itemId, bool provided) external returns (bool) {
        ...
        emit Action(itemId, "DISPUTTED", Status.DISPUTTED, msg.sender);
    ...
    function refundItem(uint256 itemId) external returns (bool) {
        ...
        emit Action(itemId, "REFUNDED", Status.REFUNDED, msg.sender);

        return true;
    }

    function withdrawFund(address to, uint256 amount) external nonReentrant returns (bool) {
        require(msg.sender == escAcc, "Only Escrow allowed");
        ...
        emit Action(block.timestamp, "WITHDREW", Status.WITHDREW, msg.sender);
